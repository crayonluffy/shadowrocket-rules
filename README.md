...
> [!IMPORTANT]
> Shadowrocket **一次只能啟用一個配置**。所以這個擋廣告配置和 AI 分流配置 `AI-Only` **只能二選一**，不能同時生效。需要 AI 就切回 `AI-Only`。
> Shadowrocket runs **only one config at a time**, so this ad-block config and the `AI-Only` config are **mutually exclusive** — switch back to `AI-Only` when you need AI access.
...
純廣告攔截清單偶爾會誤殺正常網站。先到「**資料 / Logs → 連線 / Connections**」確認該域名是不是 `REJECT`，把域名截圖傳給管理員放行即可。（暫時要用該網站時，也可先切回 `AI-Only` 或關閉連線。）
> The ad-block list occasionally catches a legit site. Check **Logs → Connections** to see if the domain shows `REJECT`, then screenshot it to the admin to allow-list. (As a quick workaround, switch back to `AI-Only` or toggle the connection off.)
...
│   └── adblock.conf              純廣告攔截配置（獨立，與上面二選一）
