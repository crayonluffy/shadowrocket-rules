...
## 🛡️ 想純擋廣告？（選用）/ Want ad-blocking only? (optional)

另有一個**獨立**的純廣告攔截配置，會在本機擋掉廣告 / 追蹤域名（約 10 萬筆），其餘流量全部直連、**不走代理、也不用選節點**。
> There's a **separate** ad-block-only config that blocks ad/tracker domains (~100k) locally. Everything else goes direct — **no proxy, no node needed**.

訂閱 URL / Subscribe URL:

```
https://raw.githubusercontent.com/crayonluffy/shadowrocket-rules/main/configs/adblock.conf
```

訂閱方式跟步驟 2 一樣（配置 → + → 訂閱配置），命名為 `AdBlock`，點選讓左邊出現勾號，再開啟連線開關即可。
> Subscribe the same way as Step 2 (Config → + → Add Subscription), name it `AdBlock`, tap it so the ✓ appears, then flip the connection switch on.

> [!TIP]
> **AI 分流 + 擋廣告可以同時生效**，不必二選一。用 Shadowrocket 的「包含配置」把兩者合併：
> 在「配置」標籤點 `AI-Only` 右邊的 **ⓘ** → 「**General / 通用**」→「**Include Configuration / 包含配置**」→ 勾選 `AdBlock`。
> 之後啟用 `AI-Only` 時就會一併套用 `AdBlock` 的攔截規則（AI 域名走 VPN、廣告域名被擋、其餘直連）。
> **You can run AI routing AND ad-blocking at the same time** — no need to choose. Combine them with Shadowrocket's *Include Configuration*: in the **Config** tab tap the **ⓘ** next to `AI-Only` → **General** → **Include Configuration** → tick `AdBlock`. Enabling `AI-Only` then also applies the `AdBlock` rules.
...
### 用 AdBlock 配置時某個網站打不開？ / A site won't load on the AdBlock config?

純廣告攔截清單偶爾會誤殺正常網站。先到「**資料 / Logs → 連線 / Connections**」確認該域名是不是 `REJECT`，把域名截圖傳給管理員放行即可。（急著用該網站時，先關閉最上方連線開關即可。）
> The ad-block list occasionally catches a legit site. Check **Logs → Connections** to see if the domain shows `REJECT`, then screenshot it to the admin to allow-list. (Need the site right now? Just toggle the connection switch off.)
...
├── configs/
│   ├── ai-only.conf              AI 分流配置（訂閱這個走 VPN）
│   └── adblock.conf              純廣告攔截配置（可單用，或被 AI 配置 include 合併）
├── ai.list                       自動產生：AI 規則 ← ai-only.conf 引用 (PROXY)
├── adblock.list                  自動產生：廣告規則 ← adblock.conf 引用 (REJECT)
└── README.md
```
...
### 廣告攔截（獨立的 adblock.conf）：誤殺放行 / 加擋 / 換清單

- **某網站被誤殺想放行**：到 `configs/adblock.conf` 的「Layer 1 誤殺放行區」加一行 `DOMAIN-SUFFIX,網站.com,DIRECT`（在 adblock RULE-SET 之上，先匹配先生效），push 後客戶端下拉刷新即可。
- **想多擋某個廣告域名**：寫進 `extras/adblock-extras.list`，會自動合併進 `adblock.list`（策略 REJECT）。
- **想換 / 加來源**：編輯 `sources/adblock.txt`。預設用 anti-AD（約 10 萬筆 `DOMAIN-SUFFIX`）；想再補 IP / 關鍵字規則，把檔內 blackmatrix7 那行的註解取消即可。
